...
<main class="prose m-auto flex size-full max-w-4xl flex-col pt-4 prose-h1:mb-0 prose-h1:mt-4">
	{#if selectedGender != null}
		...
		<h1>
			CHANGE IN THE AGE OF LABOUR MARKET EXIT FOR <span class="uppercase">{selectedGender}</span> ACROSS
			OECD COUNTRIES, 1972 vs. 2022
		</h1>
	{/if}
	...
	{:then [loadedData, { default: muze }]}
		<SlopeStrip {muze} {loadedData} selectedGender={selectedGender ?? 'Female'}></SlopeStrip>
	{/await}
</main>
